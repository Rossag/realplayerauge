...
export default function StatusPage() {
  ...
  // Refresh checkin status every time we return to this page
  useEffect(() => {
    if (session) fetchCheckinStatus()
  }, [location.key])
  ...
  const AmbienceGlows = () => <>
    ...
    {ambience.glow && <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 40%, ${ambience.glow} 0%, transparent 70%)`, pointerEvents: 'none', zIndex: 0 }} />}
  </>

  const PortraitZone = ({ size = 260, orbSize = 70 }) => (
    <div style={{ position: 'relative', width: size, height: size + 20, flexShrink: 0 }}>
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: size - 40, height: size - 40 }}>
        <CompassRose size={size - 40} />
      </div>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
        <div style={{ position: 'relative', width: 128, height: 128 }}>
          <div style={{ width: 128, height: 128, borderRadius: '50%', overflow: 'hidden', border: '2px solid #5a4520', boxShadow: '0 0 0 4px rgba(61,46,16,0.3), 0 0 24px rgba(0,0,0,0.7)', background: '#13100a' }}>
            {avatarUrl
              ? <img src={avatarUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40 }}>⚔️</div>
            }
          </div>
          <img src="/assets/frame_circle_gold.png" alt="" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 158, height: 158, objectFit: 'contain', pointerEvents: 'none', mixBlendMode: 'lighten', opacity: 0.85 }} />
        </div>
      </div>
    </div>
  )

  const StatTooltip = () => hoveredStat ? (
    <div style={{ background: 'rgba(19,14,8,0.95)', border: '1px solid #3d2e10', borderRadius: 10, padding: '8px 14px', textAlign: 'center', marginBottom: 8 }}>
      <p style={{ fontFamily: 'Cinzel, serif', fontSize: 13, color: '#c9a84c' }}>{STAT_LABELS[hoveredStat]}: {Math.round(stats[hoveredStat])}</p>
      <p style={{ fontSize: 12, color: '#5a4520', fontStyle: 'italic', marginTop: 2 }}>
        {allDeltas[hoveredStat] ? `Base 50 · ${allDeltas[hoveredStat] > 0 ? '+' : ''}${allDeltas[hoveredStat]} from effects` : 'Base 50 · no modifiers active'}
      </p>
    </div>
  ) : null

  const CheckinBanner = ({ compact = false }) => (checkinStatus.showMorning || checkinStatus.showEvening) ? (
    <button onClick={() => navigate(checkinStatus.showMorning ? '/morning-checkin' : '/evening-checkin')}
      style={{
        width: '100%', padding: compact ? '8px 12px' : '10px 14px', borderRadius: 10,
        cursor: 'pointer', textAlign: 'left', marginBottom: 12,
        background: 'rgba(61,46,16,0.3)', border: '1px solid #5a4520',
        display: 'flex', alignItems: 'center', gap: 10,
      }}>
      <span style={{ fontSize: 20 }}>{checkinStatus.showMorning ? '🌅' : '🌙'}</span>
      <div>
        <p style={{ fontFamily: 'Cinzel, serif', fontSize: 10, color: '#c9a84c', letterSpacing: '0.04em' }}>
          {checkinStatus.showMorning ? 'Dawn Briefing awaits' : 'Dusk Reckoning awaits'}
        </p>
        {!compact && <p style={{ fontSize: 12, color: '#7a6a4a', fontStyle: 'italic', marginTop: 2 }}>
          {checkinStatus.showMorning ? "Record your rest and set today's baseline" : "Seal the day and queue tomorrow's effects"}
        </p>}
      </div>
      <span style={{ marginLeft: 'auto', color: '#5a4520', fontSize: 16 }}>›</span>
    </button>
  ) : null

  const BoonsList = () => buffs.length > 0 ? (
    <div>
      <div style={{ position: 'relative', textAlign: 'center', marginBottom: 10 }}>
        <img src="/assets/divider_ornate.png" alt="" style={{ width: '100%', height: 20, objectFit: 'fill', opacity: 0.7 }} />
        <span style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', fontFamily: 'Cinzel, serif', fontSize: 9, color: '#8a6e30', letterSpacing: '0.14em', background: ambience.bg, padding: '0 10px' }}>ACTIVE BOONS</span>
      </div>
      {buffs.map(e => <EffectCard key={e.id} effect={e} />)}
    </div>
  ) : null

  const AfflictionsList = () => debuffs.length > 0 ? (
    <div>
      <div style={{ position: 'relative', marginBottom: 10 }}>
        <img src="/assets/texture_crimson.png" alt="" style={{ width: '100%', height: 22, objectFit: 'cover', borderRadius: 4, opacity: 0.85 }} />
        <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Cinzel, serif', fontSize: 9, color: '#e8c0a0', letterSpacing: '0.14em' }}>AFFLICTIONS</span>
      </div>
      {debuffs.map(e => <DebuffCard key={e.id} effect={e} />)}
    </div>
  ) : null

  const EmptyState = () => !loading && activeEffects.length === 0 ? (
    <div style={{ textAlign: 'center', padding: '24px 16px', border: '1px solid #2a1e08', borderRadius: 10 }}>
  ...
  const ShareBtn = () => (
    <button onClick={async () => { setSharing(true); await generateAndShare({ username, stats, activeEffects, avatarUrl }); setSharing(false) }}
      style={{ padding: '7px 14px', borderRadius: 6, cursor: 'pointer', background: 'rgba(61,46,16,0.4)', border: '1px solid #5a4520', fontFamily: 'Cinzel, serif', fontSize: 10, color: sharing ? '#3d9e5a' : '#c9a84c', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', gap: 6 }}>
  ...
  // ══════════════════════════════
  if (isMobile) return (
    <div style={{ minHeight: '100%', background: ambience.bg, transition: 'background 1.2s ease', fontFamily: 'Crimson Pro, Georgia, serif', position: 'relative', overflowX: 'hidden' }}>
      <AmbienceGlows />
      <div style={{ position: 'relative', zIndex: 1, padding: '16px 16px 8px' }}>

        {/* Mobile header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div>
            <h1 style={{ fontFamily: 'Cinzel, serif', fontSize: 20, fontWeight: 700, color: '#c9a84c', letterSpacing: '0.04em', lineHeight: 1 }}>{username}</h1>
            <p style={{ fontSize: 12, color: '#5a4520', fontStyle: 'italic', marginTop: 3 }}>{format(new Date(), 'EEE, d MMM')}</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {ambience.label && (
              <span style={{ fontFamily: 'Cinzel, serif', fontSize: 9, padding: '3px 10px', borderRadius: 20, background: 'rgba(61,46,16,0.4)', border: '1px solid #3d2e10', color: '#8a6e30', letterSpacing: '0.06em' }}>
                {ambience.label}
              </span>
            )}
            <ShareBtn />
        ...
        {/* Check-in banner */}
        <CheckinBanner compact />

        {/* Portrait + orbs — centred */}
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}>
          <div style={{ position: 'relative' }}>
            <button onClick={() => fileInputRef.current?.click()} style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 112, height: 112, borderRadius: '50%', overflow: 'hidden', border: '2px solid #5a4520', cursor: 'pointer', background: '#13100a', zIndex: 5, boxShadow: '0 0 20px rgba(0,0,0,0.6)' }}>
              {avatarUrl
                ? <img src={avatarUrl} alt="Portrait" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 4 }}>
                    <span style={{ fontSize: 26 }}>⚔️</span>
                    <span style={{ fontFamily: 'Cinzel, serif', fontSize: 7, color: '#4a3e28' }}>SET PORTRAIT</span>
                  </div>
              }
              {uploadingAvatar && <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 16, height: 16, border: '2px solid #c9a84c', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /></div>}
            </button>
            <PortraitZone size={240} />
          </div>
        </div>
        ...
        {/* Effects — boons then afflictions stacked */}
        {loading ? (
          ...
          <div>
            <EmptyState />
            <BoonsList />
            {buffs.length > 0 && debuffs.length > 0 && <div style={{ height: 12 }} />}
            <AfflictionsList />
          </div>
        )}

        {/* Daily quote */}
        <div style={{ margin: '16px 0 8px', padding: '10px 12px', borderRadius: 8, background: 'rgba(13,10,6,0.6)', border: '1px solid #2a1e08' }}>
          <p style={{ fontSize: 12, color: '#5a4520', fontStyle: 'italic', lineHeight: 1.5, textAlign: 'center' }}>"{DAILY_QUOTE}"</p>
        </div>
  ...
  // ═══════════════════════════════════
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', background: ambience.bg, transition: 'background 1.2s ease', fontFamily: 'Crimson Pro, Georgia, serif', position: 'relative' }}>
      <AmbienceGlows />

      {/* LEFT PANEL */}
      <div style={{ width: 180, flexShrink: 0, display: 'flex', flexDirection: 'column', padding: '20px 16px', position: 'relative', zIndex: 1, overflowY: 'auto', background: 'linear-gradient(180deg, #1a1410 0%, #120e0a 50%, #1a1410 100%)', borderRight: '2px solid #2a1e0e', boxShadow: 'inset -4px 0 12px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ position: 'relative', marginBottom: 14, display: 'flex', justifyContent: 'center' }}>
          <button onClick={() => fileInputRef.current?.click()} style={{ width: 110, height: 110, borderRadius: '50%', overflow: 'hidden', border: '2px solid #5a4520', cursor: 'pointer', background: '#13100a', position: 'relative', boxShadow: '0 0 20px rgba(0,0,0,0.6)' }}>
            {avatarUrl ? <img src={avatarUrl} alt="Portrait" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 4 }}><span style={{ fontSize: 28 }}>⚔️</span><span style={{ fontFamily: 'Cinzel, serif', fontSize: 8, color: '#4a3e28' }}>SET PORTRAIT</span></div>}
            {uploadingAvatar && <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 16, height: 16, border: '2px solid #c9a84c', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /></div>}
          </button>
          <input ref={fileInputRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={handleAvatarUpload} />
        </div>
        {ambience.label && <div style={{ textAlign: 'center', marginBottom: 12 }}><span style={{ fontFamily: 'Cinzel, serif', fontSize: 9, letterSpacing: '0.1em', padding: '3px 10px', borderRadius: 20, background: 'rgba(61,46,16,0.4)', border: '1px solid #3d2e10', color: '#8a6e30' }}>{ambience.label}</span></div>}
        ...
        <div style={{ marginTop: 'auto', padding: '10px 8px', borderRadius: 8, background: 'rgba(13,10,6,0.6)', border: '1px solid #2a1e08' }}>
          <p style={{ fontSize: 11, color: '#5a4520', fontStyle: 'italic', lineHeight: 1.5, textAlign: 'center' }}>"{DAILY_QUOTE}"</p>
        </div>
      </div>

      {/* CENTRE */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', padding: '20px 8px 8px', position: 'relative', zIndex: 1, overflowY: 'auto' }}>
        <div style={{ position: 'relative', width: 260, height: 280, flexShrink: 0, marginBottom: 8 }}>
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 220, height: 220 }}><CompassRose size={220} /></div>
          {orbPositions.map(({ stat, style }) => (
            <StatOrb key={stat} stat={stat} value={stats[stat]} style={style} onHover={setHoveredStat} onLeave={() => setHoveredStat(null)} isHovered={hoveredStat === stat} />
          ))}
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
            <div style={{ position: 'relative', width: 128, height: 128 }}>
              <div style={{ width: 128, height: 128, borderRadius: '50%', overflow: 'hidden', border: '2px solid #5a4520', boxShadow: '0 0 0 4px rgba(61,46,16,0.3), 0 0 24px rgba(0,0,0,0.7)', background: '#13100a' }}>
                {avatarUrl ? <img src={avatarUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40 }}>⚔️</div>}
              </div>
              <img src="/assets/frame_circle_gold.png" alt="" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 158, height: 158, objectFit: 'contain', pointerEvents: 'none', mixBlendMode: 'lighten', opacity: 0.85 }} />
            </div>
          </div>
        </div>
        <div style={{ minHeight: 40, marginBottom: 8, width: '100%', maxWidth: 260 }}><StatTooltip /></div>
        {loading ? <div style={{ display: 'flex', justifyContent: 'center', padding: 32 }}><div style={{ width: 20, height: 20, border: '2px solid #c9a84c', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /></div>
          : <div style={{ width: '100%', maxWidth: 380 }}><EmptyState /><BoonsList /></div>}
      </div>

      {/* RIGHT PANEL */}
      <div style={{ width: 200, flexShrink: 0, display: 'flex', flexDirection: 'column', padding: '20px 14px', position: 'relative', zIndex: 1, overflowY: 'auto', background: 'linear-gradient(180deg, #1a1410 0%, #120e0a 50%, #1a1410 100%)', borderLeft: '2px solid #2a1e0e', boxShadow: 'inset 4px 0 12px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
          {ambience.label && <div style={{ padding: '5px 10px', borderRadius: 6, textAlign: 'center', background: 'rgba(80,16,16,0.4)', border: '1px solid rgba(139,26,26,0.5)', fontFamily: 'Cinzel, serif', fontSize: 10, color: '#c05050', letterSpacing: '0.06em' }}>{ambience.label.toUpperCase()}</div>}
          <ShareBtn />
        </div>
        <div style={{ height: 1, background: 'linear-gradient(to right, transparent, #3d2e10, transparent)', marginBottom: 14 }} />
        <AfflictionsList />
        {debuffs.length === 0 && <div style={{ textAlign: 'center', padding: '16px 8px' }}><p style={{ fontSize: 20, marginBottom: 6 }}>🛡️</p><p style={{ fontFamily: 'Cinzel, serif', fontSize: 10, color: '#3d2e10', letterSpacing: '0.06em' }}>No afflictions</p></div>}
